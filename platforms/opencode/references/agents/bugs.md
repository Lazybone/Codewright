...
- **Off-by-one errors** in loops (e.g., `< length` vs `<= length`, starting at 0 vs 1)
- **Incorrect operator precedence** (e.g., `a & b == c` where `a & (b == c)` was not intended)
- **Deadlock potential** with nested locks or inverted lock ordering
...
async calls are correctly awaited. Also check for deadlock potential
with nested locks or inverted lock ordering.
...
property access is a bug. Check for missing null checks and missing
default values at critical locations.

### 5. Resource Leaks

Look for resources that are acquired but never properly released:

- **Opened files/connections without close** -- file handles, database connections,
  network sockets opened but not closed in a `finally` block or equivalent
- **Missing context managers** -- Python: using manual `open()`/`close()` instead
  of `with` statements; other languages: missing try-with-resources or `using`
- **Event listeners without cleanup** -- registered event handlers that are never
  removed, leading to memory leaks (especially in long-running processes or SPAs)

```bash
# Python: open() without with statement
grep -rniE '^\s*\w+\s*=\s*open\(' --include="*.py" . 2>/dev/null | grep -v node_modules

# JS/TS: addEventListener without removeEventListener
grep -rniE 'addEventListener\(' --include="*.{ts,tsx,js,jsx}" . 2>/dev/null \
  | grep -v node_modules | grep -v '.git/'

# Go: opened resources (check for matching defer/close)
grep -rniE '\b(Open|Dial|Listen)\(' --include="*.go" . 2>/dev/null | grep -v '_test.go'
```
...
- **Category**: logic / error-handling / async / null-safety / resource-leak / deprecated / type-safety / lint
...
- `auto` -- can be fixed programmatically (e.g., adding missing `await`, replacing bare `except` with specific exception, adding null checks, closing resource handles)
- `manual` -- requires human judgment (e.g., race conditions, architecture-level bugs, complex logic errors)
- `info` -- informational hint, no direct code fix
